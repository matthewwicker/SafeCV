...
def untargeted_reward_function(old, new):
	return old - new

def targeted_reward_function(old, new, target):
	return new[0][target] - old[0][target]
...
def MCTS(params):
    ...
    if((pred != int(params.TRUE_CLASS) and params.target_class != None) or (pred == params.target_class and params.target_class != None)):
	if(params.verbose):	
		print("Image was initially misclassified")
	return params.ORIGINAL_IMAGE, 0, params.SOFTMAX, ([],[],[])
    ...
    while(True):
		...
		print("Satisfied before simulation")
			...
			reward = untargeted_reward_function(params.PROBABILITY, NEW_PROBABILITY)
		    else:
			reward = targeted_reward_function( params.SOFTMAX, NEW_SOFTMAX, params.target_class)
                    visited[i].backprop(visited[i+1].id, reward, current_level+1)
	    ...
	    if(MISCLASSIFIED == True):
		if(current_level == 0):
		    print("Single manipulation adversary found... breaking")
		    break
		print("Satisfied before simulation")
      		adv = copy.deepcopy(IMAGE)
             	softmax = copy.deepcopy(prob)
